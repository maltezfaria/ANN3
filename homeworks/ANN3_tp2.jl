...
using LinearAlgebra, Plots, PlutoUI, Test, BenchmarkTools
...
md"""!!! danger "Submission instructions"
	**Due date:** You do not need to submit this homework
...
In this homework we will get some hands-on experience on implementing a direct solver (LU), and study some of its stability properties. We will conclude by looking at an application where we must invert dense linear systems, and a few surprises that come along the condition number. 

You are encouraged to consult the course material available [here](https://hal.science/hal-03321502v2/file/aln.pdf). Feel free to reference the formulas in the course material if needed. 

Let's get started. 
"""

# ╔═╡ 886e770d-3cff-4e6a-a4c5-94ed7ad9b28e
md"""

## Triangular solve

Firstly, we need to be able to solve triangular systems. 

"""

# ╔═╡ 23134fbd-aa07-4e25-a5ec-0edd2d807997
...
# ╔═╡ 54e7344a-9aee-473c-abb0-c81c39a939b1
...
# ╔═╡ 3a3e1e4b-36f8-4951-bf43-e1bc079f1f1c
function forward_substitution!(b, A)
    
    return x
end

# ╔═╡ 343f8c6d-6b40-4011-855c-079f9bc503ea
md"""
The block below will check that your function does not allocated:
"""

# ╔═╡ e200184f-81ca-4f97-8c3f-808105e0055d
try
	n = 3
	A = rand(n,n)
	b = rand(n)
	println("Allocations on problem of size $n x $n:")
	alloc = @allocated forward_substitution(A,b)
	println("\t forward_substitution:  $alloc bytes")
	alloc = @allocated forward_substitution!(b,A)
	println("\t forward_substitution!: $alloc bytes")
catch 
	@warn "Error when trying to measure allocations"
end

# ╔═╡ b92c7a11-4b98-4538-a6b1-0664dddabc82
md"""
!!! danger "Allocations and performance"
	Although they account for no flops, memory allocations can be a major bottleneck in many algorithms, so whenever possible it is good to keep their numbers down. 
"""

# ╔═╡ cbb725dd-2c01-4935-aee7-de572f4b2c94
md"""
## LU factorization

We now tackle the LU factorization of a matrix. Your first job is to understand/derive algorithm 2.3 of the [lecture notes](https://hal.science/hal-03321502v2/file/aln.pdf). 
"""

# ╔═╡ 5656dc93-1ff2-4c30-b96e-12100c76fed9
function basic_lu!(A)
	n, m = size(A)
	for k in 1:n-1
		for i = k+1:n
...
# ╔═╡ ec7cefe6-bcbd-4fe7-95c6-1a092479b2f9
function row_pivot_basic_lu!(A)
    n, m = size(A)
    P = collect(1:n) # row permutation vector
	# many loops later...
    return P, A
end
...
In this last part we will focus on an application on *polynomial interpolation* and its connection to *numerical linear algebra*. More precisely, we will investigate the following problem:
...
# ╔═╡ ac3eece0-8205-4d49-8f98-08f5b4922cf3
let
	n = 10
	...
	@show norm(p.(x)-f.(x),Inf)
end

# ╔═╡ d7e82187-d508-4447-a151-314f85749517
md"""
The choice of interpolation points ``\mathcal{I}_n = \{ x_i \}_{i=0}^n`` plays an important role in the quality of the approximation (assuming exact arithmetic), while the choice of basis for ``\mathbb{P}_k`` affects the conditioning of the interpolation method (i.e. how well can you actually find the unique polynomial you seek in the presence of rounding errors). 

Defining ``\Pi : C[-1,1] \to \mathbb{P}_n`` as the map from the space of continuous function to the resulting (unique) polynomial in ``\mathbb{P}_n`` that interpolates the function at $\mathcal{I}_n$. We may define its operator norm as
```math
\Lambda(\mathcal{I}_n) = \sup_{||f||=1} || \Pi(f)||
```
where $||\cdot||$ will denote the supremum norm. The constant ``\Lambda(\mathcal{I}_n)`` is commonly refered to as its **Lebesgue constant** for ``\mathcal{I}_n``. It is a simple exercise to show that
```math
...
let
	n = 30
	scatter([-cos((2k-1)/2n*π) for k in 1:n], zeros(n); label="chebyshev")
	ylims!(-0.5,0.5)
end
...
# ╔═╡ 4ad9baa5-4368-4dc5-b0fa-30a8afa7de55
p = lagrange_interpolation(rand(10),rand(10))

# ╔═╡ 8fde2a0a-9dcd-4b81-b7ae-4249587ebaac
md"""
Here is a figure that shows how the interpolation error changes as a function of $n$ when using two ways of computing the interpolating polynomial $p_n$ given a set of Chebyshev nodes. (this figure only makes sense if your previous code is working)
"""
...
let
	...
	#plot!(twinx(),ccmax, lc=:yellow, lw=4, yscale=:log10, label="", ls=:dash)
end
...
# ╔═╡ 24373de6-7ed8-483e-8190-6e7028912f23
question(md"""		 
Complete the `forward_substition` function below to return $x$, solution of $Ax = b$, where $A$ is a lower-triangular matrix. 
""")

# ╔═╡ c1698286-dc76-46e3-b943-ca061090395a
question(md"""		 
Complete the `backward_substition` function below to return $x$, solution of $Ax = b$, where $A$ is an upper-triangular matrix. 
""")

# ╔═╡ fffa3907-4bfe-47f7-826b-3044869bb31b
question(md"""
Complete the function `forward_substitution!(b,A)` to return the solution $x$ to $Ax = b$ using the memory in $b$. This function should not have any allocation.
"""	
)

# ╔═╡ 26f36f6a-2524-43b4-84ef-141f4775b159
question(md"""
Try to make your `forward_substitution` function as fast as possible. Some tips:
1. Be careful how you access your matrix: **data locality** is important
2. If you are sure indices are inbounds, you can tell the compiler that using `@inbounds` (see the docs)
3. Use the `BenchmarkTools` package and the `@benchmark` macro for statistically significant measurements
""", "(bonus)")

# ╔═╡ f09a9c0d-740a-4b25-9649-3783b8fc8782
question(md"""
Understand/derive algorithm 2.3 of the [lecture notes](https://hal.science/hal-03321502v2/file/aln.pdf). Write the scalar/vector product there as a simple for loop. 
""")

# ╔═╡ 60480590-c0fe-4b71-8a2c-b9b336b4f923
question(md"""
Complete the `basic_lu!` function below to replace a matrix by its $LU$ factorization. The $U$ part should be stored in the upper triangular part of $A$, while $L$ should be stored in the (strick) lower triangular part of $A$ with the convention that the diagonal part of $L$ is one (i.e. $L$ is unit lower triangular).
Try not to allocate any dynamic memory. 
""")

# ╔═╡ ccd62be6-091e-4ab9-a9dd-33ef9fd7e7e6
question(md"""
Once your `basic_lu!` works, try it on the following matrix:
```math
\begin{bmatrix} 
		 1 & 2 & 1 & 1 \\ 
		 1 & 3 & 2 & 2 \\
...
# ╔═╡ 9d85235b-2ffe-407c-8de9-fe83245c0f43
question(md"""
Modify the `basic_lu` function to perform row pivoting, and try factoring the matrix above again.		 
""")

# ╔═╡ 59937c0d-e2f3-4217-ae6f-9ff9664cda99
question(md"""
There are may possible ways to order the three loops in the $LU$ factorization algorithm. Experient different possible orders, and see if there are any significan impacts on performance. Remember to always test your implementation for correctness before measuring time!
""", "(Bonus)")
...
question(md"""
Interporlating a smooth function on Chebyshev nodes should converge exponentially fast as $n \to \infty$. Explain the behaviour of the code below given what you have learned about linear algebra. Does the error decay to machine precision? Is it even true that ``p(x_i) = f(x_i)`` for the interpolation nodes?
""")

# ╔═╡ 10116502-8a73-45ca-ab72-8e5b6ce6d115
question(md"""
Given what you know about linear algebra, assuming a *backward stable* method is used to solve the linear system ``Vc = b``, where ``V`` is the Vandermond matrix and ``c`` the sought polynomial coefficients, what can you say about the residual ``||V\tilde{c} - b||``, where $\tilde{c}$ denotes the output of the algorithm (i.e. the approximate solution)?
""")
...
# ╔═╡ d9858621-261d-4939-82c0-dcb8a80a6ed6
answer(md"""
Answer here. Be sure to explain your reasoning.
""")
...
# ╔═╡ a8aa0e5b-55ed-4828-8067-752abea4fdc2
check_answer() do
	@testset "Forward substitution" begin
		n = 10
		A = LowerTriangular(I + rand(10,10))
		b = rand(n)
		x = LowerTriangular(A) \ b
		x_ = forward_substitution(Matrix(A),b)
		@test norm(x - x_, Inf) < 1e-8
	end
end

# ╔═╡ f076c9ff-eee1-472f-86a5-772df663a8fd
check_answer() do
	@testset "Backward substitution" begin
		n = 10
		A = UpperTriangular(1 .+ rand(10,10))
		b = rand(n)
		x = A \ b
		x_ = backward_substitution(Matrix(A),b)
		@test norm(x - x_, Inf) < 1e-8
	end
end

# ╔═╡ dca88685-2534-4302-ac96-e38ba72c0480
check_answer() do
	@testset "Forward substitution (inplace)" begin
		n = 10
		A = LowerTriangular(I + rand(10,10))
		b = rand(n)
		x = LowerTriangular(A) \ b
		x_ = forward_substitution!(b, Matrix(A))
		@test norm(x - x_, Inf) < 1e-8
		forward_substitution!(b, A)
		a = @allocated forward_substitution!(b, A)
		@test iszero(a)
	end
end

# ╔═╡ e49b6275-1443-4011-b05d-4bbf286c4e36
check_answer() do
	@testset "Simple LU" begin
		n = 10
		A = I + 0.1*rand(n,n)
		F = lu(A, NoPivot())
		A = basic_lu!(A)
		@test norm(F.U - UpperTriangular(A)) < 1e-8
		@test norm(F.L - UnitLowerTriangular(A)) < 1e-8
		alloc = @allocated basic_lu!(A)
		@test iszero(alloc)
	end
end

# ╔═╡ cd4ceac2-810c-4b2a-b72e-6e639e96f3f4
check_answer() do
	@testset "LU row pivot" begin
		n = 10
		A = I + 0.1*rand(n,n)
		F = lu(A)
		A = basic_lu!(A)
		@test norm(F.U - UpperTriangular(A)) < 1e-8
		@test norm(F.L - UnitLowerTriangular(A)) < 1e-8
		row_pivot_basic_lu!(A)
		alloc = @allocated row_pivot_basic_lu!(A)
		#@test iszero(alloc)
	end
end
...
manifest_format = "2.0"
project_hash = "23fc3701c99dbbc47053394fe282d91784b3d038"
